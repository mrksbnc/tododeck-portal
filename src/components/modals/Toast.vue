<template>
  <transition name="fade">
    <div class="grid·items-center·pointer-events-auto·cursor-pointer">
      <div
        v-if="visible"
        :key="id"
        class="flex relative w-full h-16 max-w-sm mx-auto overflow-hidden right-0 bg-white mx-1 my-1 rounded-lg shadow-md dark:bg-gray-800 top-0 z-50 toaster cursor-pointer"
        @mouseover="pauseToastHide()"
        @mouseout="resumeToastHide()"
      >
        <span
          class="absolute flex text-gray-500 h-full right-1 top-1 ml-2 mr-2 cursor-pointer hover:text-red-500"
          @click="hideToast()"
        >
